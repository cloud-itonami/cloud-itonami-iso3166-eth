# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Ethiopia

## Classification

- Repository: `cloud-itonami-iso3166-eth`
- ISO 3166: `ETH` (Ethiopia)
...
- Social impact: [:ethiopian-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on an Ethiopian
  public contract
...
- registration walkthrough for the e-GP (Electronic Government
  Procurement) portal (egp.ppa.gov.et), rolled out by the Federal Public
  Procurement and Property Authority (PPPA) and expanding across federal
  agencies since its 2021 pilot
- business/tax registration checklist: business registration with the
  Ministry of Trade and Regional Integration (or the relevant regional
  trade bureau) and a Taxpayer Identification Number (TIN) required for
  e-GP supplier sign-up
- local-content / preferential-procurement navigation: Ethiopian public
  procurement directives' domestic-preference provisions for locally
  sourced goods and Ethiopian contractors on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Ethiopian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Ethiopia).
